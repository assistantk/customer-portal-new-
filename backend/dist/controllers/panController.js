...
export const uploadPanFile = asyncHandler(async (req, res) => {
    ...
    let existingRows = await executeQuery(`SELECT pan_file_name, pan_file_type, pan_file_path, pan_number, pan_verification_status FROM customer_code WHERE customer_code = ?`, [customerCode]);
    if (!existingRows.length) {
        tableName = 'handling_agents';
        codeCol = 'handling_code';
        existingRows = await executeQuery(`SELECT pan_file_name, pan_file_type, pan_file_path, pan_number, pan_verification_status FROM handling_agents WHERE handling_code = ?`, [customerCode]);
    }
    if (!existingRows.length)
    ...
    await executeQuery(`UPDATE ${tableName} SET pan_file_name = ?, pan_file_type = ?, pan_file_path = ?, pan_number = COALESCE(?, pan_number), pan_verification_status = 'VERIFIED' WHERE ${codeCol} = ?`, [file.originalname, file.mimetype, filePath, scan.pan, customerCode]);
