...
export const getStateNameFromCode = (code: string): string | null => {
  if (!code) return null;
...
export const correctPanOcr = (candidate: string): string | null => {
...
export const correctGstinOcr = (candidate: string): string | null => {
  if (!candidate) return null;
  const clean = candidate.trim().toUpperCase().replace(/[^A-Z0-9]/g, '');
