...
export const findNextUniqueSuffix = (
  existingCodes: string[],
  base: string,
  maxLength: number = env.CODE_MAX_LENGTH,
  maxRetries: number = env.CODE_MAX_RETRIES
): string | null => {
  const baseTrim = base.slice(0, maxLength);
  if (!existingCodes.includes(baseTrim)) return baseTrim;
  ...
  for (let n = 1; n <= maxRetries; n++) {
    ...
    if (suffix.length > maxLength - basePrefix.length) continue;
...
export const generateUniqueCode = async (
  ...
  if (!finalCode) {
    throw new Error(
      `Could not generate a unique ${type} code for "${companyName}" after ${env.CODE_MAX_RETRIES} attempts. ` +
...
export const reserveUniqueCode = async (
  companyName: string,
  type: CodeType,
  maxLength: number = env.CODE_MAX_LENGTH
): Promise<{ code: string; base: string; variationsTried: number }> => {
